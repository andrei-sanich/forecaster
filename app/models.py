...
class User(UserMixin, db.Model):
    ...
    def to_json(self):
        json_user = {
            ...
            'points': self.points,
            'forecast_count': self.forecasts.count()
        }
        return json_user
...
league_team = db.Table(
    'league_team',
    db.Column('team_id', db.Integer, db.ForeignKey('teams.team_id')),
    db.Column('league_id', db.Integer, db.ForeignKey('leagues.league_id')))


class Team(db.Model):
    __tablename__ = 'teams'
    team_id = db.Column(db.SmallInteger, primary_key=True)
    name = db.Column(db.String(64))
    leagues = db.relationship(
        'League',
        secondary=league_team,
...
class League(db.Model):
    ...
    league_id = db.Column(db.SmallInteger, primary_key=True)
    name = db.Column(db.String(64))
...
class Match(db.Model):
    ...
    home_goal = db.Column(db.SmallInteger, nullable=True)
    guest_goal = db.Column(db.SmallInteger, nullable=True)
    is_actual = db.Column(db.Boolean, default=True)
    forecasts = db.relationship('Forecast', backref='match', lazy='dynamic')

    def __repr__(self):
        return '<{} - {} {}>'.format(
            self.home_team, self.guest_team, self.timestamp)

            
class Forecast(db.Model):
    __tablename__ = 'forecasts'
    id = db.Column(db.Integer, primary_key=True)
    home_goal = db.Column(db.SmallInteger)
    guest_goal = db.Column(db.SmallInteger)
    user_id = db.Column(db.Integer, db.ForeignKey('users.id'))
    match_id = db.Column(db.Integer, db.ForeignKey('matches.id'))

    def __repr__(self):
        return '<{} - {}>'.format(self.home_goal, self.guest_goal)
